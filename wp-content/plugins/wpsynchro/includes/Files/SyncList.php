...
class SyncList
{
    ...
    public function getFilesToMoveToTarget($max_size)
    {
        ...
        // Get work from DB
        global $wpdb;
        $files_chunk = $wpdb->get_results("SELECT `id`, `section`, `source_file`, `is_dir`, `size`, `hash`, `is_partial`, `partial_position` FROM `{$wpdb->prefix}wpsynchro_sync_list` WHERE `needs_transfer`=1  ORDER BY `source_file` ASC LIMIT 100");
